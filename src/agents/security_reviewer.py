"""
Security Reviewer Agent — Day 3 + TODO-SEC-01 (Semgrep multi-language SAST).

Agent loop:
  1. 查询长期记忆，将同仓库历史 findings 注入 Prompt 作为额外上下文
  2. LLM with bound tools 决定调用哪些工具（semgrep_scan / scan_secrets）
...
_TOOL_MAP = {t.name: t for t in _SECURITY_TOOLS}
...
def _agent_loop(
    ...
    llm_with_tools = llm.bind_tools(_SECURITY_TOOLS)
