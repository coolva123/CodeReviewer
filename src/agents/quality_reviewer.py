"""
Quality Reviewer Agent — Day 3 + TODO-QUAL-01 (Semgrep maintainability + Ruff).

Agent loop:
  1. 查询长期记忆，将同仓库历史 findings 注入 Prompt 作为额外上下文
  2. LLM with bound tools 决定调用哪些工具（ast_analyze / semgrep_scan / ruff_check）
  3. 所有工具调用经由 Tool Guard 路由（风险分级 + HITL）
  4. 工具结果喂回对话；构建"干净消息"避免 DeepSeek reasoning_content 问题
  5. call_structured 提取结构化 findings
  Fallback: 静态运行所有工具 → LLM 分析。
"""
import json
import logging
...
_QUALITY_TOOLS = [ast_analyze, semgrep_scan, ruff_check]
_TOOL_MAP = {t.name: t for t in _QUALITY_TOOLS}
...
def _agent_loop(
    ...
    llm_with_tools = llm.bind_tools(_QUALITY_TOOLS)
